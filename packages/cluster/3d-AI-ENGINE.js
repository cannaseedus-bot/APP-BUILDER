// K'UHUL THREE.JS RENDER ENGINE - Complete 200+ Line Implementation
class KuhulThreeJSEngine {
    ...
    setupDefaultScene() {
        ...
        renderer.setClearColor(0x0a0a1a); // K'UHUL dark theme
        renderer.shadowMap.enabled = true;
        renderer.shadowMap.type = THREE.PCFSoftShadowMap;
        
        this.scenes.set('default', { scene, camera, renderer });
    ...
    initThreeJS(containerId) {
        ...
        renderer.setClearColor(0x070b12); // K'UHUL panel color
        container.appendChild(renderer.domElement);

        this.scenes.set(containerId, { scene, camera, renderer });
    ...
    create3DText(text, options = {}) {
        ...
        this.meshes.set(`text_${Date.now()}`, textMesh);
...
// K'UHUL THREE.JS BRIDGE - Your symbolic language to Three.js
...
// GLOBAL K'UHUL 3D ENGINE
window.Kuhul3D = new KuhulThreeJSBridge();

// AUTO-INITIALIZE ON PAGE LOAD
document.addEventListener('DOMContentLoaded', () => {
    console.log('🎮 K\'UHUL 3D ENGINE READY');
    ...
    if (!document.getElementById('three-container')) {
        ...
        container.style.background = '#070b12';
        document.body.appendChild(container);
    }
    
    // Initialize engine
    window.Kuhul3D.engine.initThreeJS('three-container');
    
    // Your original K'UHUL script
    const kuhulScript = `
        [Pop render_3d_text] [Wo "K'uhul"]→[Ch'en text] 
    ...
    // Execute the script
    window.Kuhul3D.processSymbolicCode(kuhulScript);
});