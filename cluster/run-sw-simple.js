...
/**
 * XJSON SW.JS STANDALONE RUNNER (CommonJS Version)
 * 
 * No package.json needed - works immediately!
 * 
 * Usage:
 *   node run-sw-simple.js
 *   node run-sw-simple.js --cluster --workers 8
 */
...
const config = {
  ...
  swPath: process.argv.find(a => a.startsWith('--sw='))?.split('=')[1] || './public/ml-runtime-sw.js',
...
class ServiceWorkerRuntime {
  ...
  async compress(data) {
    ...
    const compressedSize = Math.floor(originalSize * 0.13);
...
async function main() {
  console.log('\n🚀 XJSON SW.JS STANDALONE RUNNER (CommonJS)\n');
